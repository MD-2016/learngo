...
func TestConvertToRomanNumeral(t *testing.T) {
	for _, test := range cases {
		t.Run(fmt.Sprintf("%d gets converted %s", test.Arabic, test.Roman), func(t *testing.T) {
...
func TestConversion(t *testing.T) {
	assert := func(arab uint16) bool {
		if arab > 3999 {
			return true
		}
		t.Log("testing", arab)
		roman := ArabicToRoman(arab)
		fromRoman := RomanToArabic(roman)
		return fromRoman == arab
	}

	if err := quick.Check(assert, &quick.Config{
		MaxCount: 1000,
	}); err != nil {
